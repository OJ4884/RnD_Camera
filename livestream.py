...
def start_recording():
    ...
    try:
        ...
        recording_output = FfmpegOutput(filename)
        encoder = H264Encoder(bitrate=10000000)
        camera.start_recording(encoder, recording_output)
        recording = True
...
def frame_capture_thread():
    ...
    while streaming:
        try:
            ...
            if request_obj:
                with frame_lock:
                    frame_buffer = request_obj.make_buffer("main")
                request_obj.release()
                time.sleep(0.033)  # ~30fps
        except Exception as e:
...
def initialize_camera(resolution=(1920, 1080), fps=30):
